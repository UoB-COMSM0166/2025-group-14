...
=======
class canal {
  constructor(width, name, startX, startY, endX, endY, waterTileFrame) {
    this.waterTileFrame = waterTileFrame;
    //preliminary maths
    let a = angleCalc(startX, startY, endX, endY);
    let opp = Math.sin(a) * width;
    let adj = Math.cos(a) * width;
...
>>>>>>> p5Play_pursuer
  ...
  setBeforeThreshold() {
    ...
    const x1 = linearIntersectX(
      redGrad,
      redOff,
      targRed.gradient,
      targRed.offset
    );
    const y1 = linearIntersectY(
      redGrad,
      redOff,
      targRed.gradient,
      targRed.offset
    );
    const x2 = linearIntersectX(
      blackGrad,
      blackOff,
      targBlack.gradient,
      targBlack.offset
    );
    const y2 = linearIntersectY(
      blackGrad,
      blackOff,
      targBlack.gradient,
      targBlack.offset
